...
class SimplePdfApp:
    ...
    def show_thumbnail_view(self):
        ...
        info_text = ("缩略预览：左键单击选中页面；【右键单击缩略图】弹出菜单：删除本页 / 移动到指定页码；"
                     "输入N移动后严格为第N页；缩略图自动裁掉四周空白仅为预览；导出PDF完整保留设置的纸张尺寸；滚轮滚动；可改宽度重渲染；可返回源文件列表")
    ...
    def _build_preview_from_raw(self, base_w):
        """
        重大BUG修复：就地过滤无效索引，**绝不直接覆盖self.page_indices，保护用户手动调整的页面顺序**
        """
        try:
            self.source_doc.close()
        except Exception:
    ...
    def on_merge(self):
        ...
        try:
            ...
            if len(self.raw_pages) == 0:
                messagebox.showerror("合并失败", "没有读到任何有效页面，文件可能损坏/加密。")
                return
